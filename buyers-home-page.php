...
if (isset($_SESSION['username'])) {
    ...
    <body>
                        ...
                        <div class='item' onclick="showModal(<?= htmlspecialchars(json_encode($row)) ?>)">
                            ...
                            <div class='basic-info'>
                                ...
                                <div class='item-price'>
                                    <?php echo htmlspecialchars($row['price']); ?> LKR/kg
                                </div>
                                <?php if ($isDonated): ?>
        ...
        <script>
            var modal = document.getElementById("myModal"); var modalText = document.getElementById("modal-text"); var span = document.getElementsByClassName("close")[0];
            function showModal(item) { var content = '<div><strong>Name:</strong> ' + item.name + '</div>' + '<div><strong>Weight:</strong> ' + item.weight + ' kg</div>' + '<div><strong>Price:</strong> ' + item.price + ' LKR/kg</div>' + '<div><strong>Contact:</strong> ' + item.contact + '</div>' + '<div><strong>Address:</strong> ' + item.address + '</div>' + '<div><strong>Ratings:</strong> ' + item.ratings + ' / 5 Stars</div>' + '<div><strong>Date:</strong> ' + new Date(item.created_at).toLocaleString() + '</div>'; modalText.innerHTML = content; modal.style.display = "block"; }
            span.onclick = function () { modal.style.display = "none"; }
            window.onclick = function (event) { if (event.target == modal) { modal.style.display = "none"; } }
            function filterCategory(category) { const searchTerm = new URLSearchParams(window.location.search).get('search') || ''; window.location.search = `?category=${category}&search=${searchTerm}`; }


            function openChatWithSeller(sellerId) { window.location.href = `chat/chat.php?user=${sellerId}`; }
        </script>
